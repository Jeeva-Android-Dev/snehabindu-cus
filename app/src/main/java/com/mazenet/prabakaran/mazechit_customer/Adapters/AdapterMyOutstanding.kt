...
class AdapterMyOutstanding (
    private val itemsList: ArrayList<MyChitsModel>,
    private val listener: IAdapterClickListener
    ) : RecyclerView.Adapter<AdapterMyOutstanding.MyViewHolder>() {

        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
            val view = LayoutInflater.from(parent.context)
                .inflate(R.layout.outstandinglist_item, parent, false)
            return MyViewHolder(view, listener)
        }

        override fun onBindViewHolder(holder: MyViewHolder, position: Int) {

            holder.txt_chitvalue.text= Constants.money_convertor(
                Constants.isEmtytoZero(
