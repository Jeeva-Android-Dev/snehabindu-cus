...
class Groupwise_Receipts : BaseFragment() {
    ...
    override fun onCreateView(
        ...
        txt_grpname.text = ": " + arguments!!.getString("groupname") + " / " + arguments!!.getString("ticketno")
        pb_grprecpts = view.findViewById(R.id.pb_grprecpts) as ProgressBar
        Recycler_grprecpts.setHasFixedSize(true)
        val mLayoutManager = LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false)
        Recycler_grprecpts.layoutManager = mLayoutManager
        Recycler_grprecpts.itemAnimator = DefaultItemAnimator()
    ...
    fun integrateList(leadslist: ArrayList<AccountCopyData>) {
        ...
        for (i in Grouplist) {
            if (i.getReceiptType().equals("Due")) {
               // total_collected = total_collected + i.getoverallReceivedAmount()!!.toInt()
                total_collected = i.getoverallReceivedAmount()!!.toInt()
            }
        }
        txt_total.text = ": " + Constants.money_convertor(Constants.isEmtytoZero(total_collected.toString()), false)
        Recycler_grprecpts.adapter = grprecpts_adapter
    }
